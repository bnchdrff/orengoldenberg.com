...
DataHelper.prototype.formatVideos = function() {
  ...
  _.each(this.videos, function(video) {
    _.each(video.pictures.sizes, function(picture) {
      picture.link = picture.link.replace('i.vimeocdn.com', 'www.orengoldenberg.com/images').replace('https', 'http');
    });
  });
...
DataHelper.prototype.loadVideos = function(fromCache) {
  if (!fromCache && process.env.NODE_ENV == 'production') {
    ...
    try {
      this.setVideos(require(__dirname + '/../assets/testdata.json'));
    } catch (e) {
